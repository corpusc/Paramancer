...
public class LevelLight : MonoBehaviour {
	...
	void Update()
	{
		if(MovingRandomly)
		{
			...
			if(Vector3.Distance(light.transform.position, p_origin) > MaxWanderDistance) light.transform.position = p_origin; //tele to start pos
		}
		if(Flickering)
		{
			if(Random.Range(0, FlickerTimer) == 0) light.enabled = !light.enabled;
		}
	}
}
